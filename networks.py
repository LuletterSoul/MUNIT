...
class AdaINGen(nn.Module):
    ...
    def assign_sanet_params(self, style_code, model):
        # assign the adain_params to the AdaIN layers in model
        for m in model.modules():
            if m.__class__.__name__ == "SANet":
                m.style_code = style_code
...
class SADecoder(nn.Module):
    def __init__(self, n_upsample, n_res, style_dim, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero'):
        ...
        for i in range(n_upsample):
            self.model += [nn.Upsample(scale_factor=2),
                           SAConv2dBlock(style_dim, dim, dim // 2, 5, 1, 2, norm='ln', activation=activ,
                                         pad_type=pad_type)]
            dim //= 2
        # use reflection padding in the last conv layer
        self.model += [
            SAConv2dBlock(style_dim, dim, output_dim, 7, 1, 3, norm='none', activation='tanh', pad_type=pad_type)]
        self.model = nn.Sequential(*self.model)
